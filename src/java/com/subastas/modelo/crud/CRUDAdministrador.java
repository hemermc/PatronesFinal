...
public class CRUDAdministrador implements ICRUDGeneral<Administrador>, ICRUDUsuario {
    ...
    public void eliminar(String nombre_usuario) {
        String consulta = "DELETE FROM Administradores WHERE " + Constantes.NOMBRE_USUARIO + " = ?";//Genera la consulta
    ...
    public void actualizar(Administrador administrador) {
        String consulta = "UPDATE Administradores "
                + "SET " + Constantes.CLAVE + " = ?, " + Constantes.NIVEL_PERMISOS + " = ? WHERE " + Constantes.NOMBRE_USUARIO + " = ?";
        try (PreparedStatement ps = conexion.prepareStatement(consulta)) {
    ...
    public Administrador obtenerEspecifico(String nombre_usuario) {
        ...
        String consulta = "SELECT * FROM Administradores WHERE " + Constantes.NOMBRE_USUARIO + " = ?";
        try (PreparedStatement ps = conexion.prepareStatement(consulta)) {
            ps.setString(1, nombre_usuario);
    ...
    public Administrador formatearResultado(ResultSet rs) {
        ...
        try {
            administrador = new Administrador(
                    ...
                    rs.getInt(Constantes.NIVEL_PERMISOS));
        } catch (SQLException ex) {
    ...
    public boolean inicioSesionValido(Usuario usuario) {
        String consulta = "SELECT * FROM Administradores WHERE " + Constantes.NOMBRE_USUARIO + " = ? AND " + Constantes.CLAVE + " = ?";
        boolean valido = false;
        try (PreparedStatement ps = conexion.prepareStatement(consulta)) {
